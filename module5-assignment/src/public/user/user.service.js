...
function UserService() {
  ...
  service.hasData = function(){ return data != null; }
  service.getData = function(){ return data; }

  service.save = function(userData){
    if(!userData){ throw new Error('Data cannot be empty!'); }
  ...
  function getProperty(obj, name, required){
    if(!obj[name] && required){ throw new Error('"' + name + '" data is required'); }
    return obj[name];
  }
}
